...
FOR
INSERT WITH CHECK (NOT (auth.uid() = ANY(players_id_array)));


CREATE POLICY update_match_policy ON sm_game.tb_match
FOR
UPDATE USING (auth.uid() = ANY(players_id_array));


CREATE POLICY select_own_match ON sm_Game.tb_match
FOR
SELECT USING (auth.uid() = ANY(players_id_array));
...
CREATE OR REPLACE FUNCTION sm_Game.func_create_match_if_not_in_any(player_id UUID, player_email VARCHAR(100)) RETURNS VOID AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION sm_Game.func_join_match_if_not_in_any(player_id UUID, player_email VARCHAR(100), match_id INT) RETURNS VOID AS $$
BEGIN
    -- Check if the player is already in a match
    IF NOT EXISTS (
        SELECT 1 FROM sm_Game.tb_match WHERE player_id = ANY(players_id_array)
